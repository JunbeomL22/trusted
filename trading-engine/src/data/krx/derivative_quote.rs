...
impl IFMSRPD0034 {
    ...
    pub fn to_quote_snapshot_buffer(
        ...
        buffer.system_timestamp.stamp = get_unix_nano();
        buffer.timestamp = timestamp_converter.parse_hhmmssuuuuuu(
            &payload[self.timestamp_slice.start..self.timestamp_slice.end], 
            Some(buffer.system_timestamp),
            date_gen,
        )?;
...
impl IFMSRPD0035 {
    ...
    pub fn to_quote_snapshot_buffer(
        &self, payload: &[u8], buffer: &mut QuoteSnapshot, date_gen: &mut DateUnixNanoGenerator,
    ) -> Result<()> {
        self.is_valid_krx_payload(payload)?;
        self.is_valid_quote_snapshot_buffer(payload, buffer)?;
...
mod tests {
    ...
    fn test_parse_ifmsrpd0034() -> Result<()> {
        ...
        let mut date_gen = &mut &mut DateUnixNanoGenerator::from(
            date!(2023-12-30)
        );
    ...
    fn test_parse_ifmsrpd0034_with_buffer() -> Result<()> {
        ...
        let mut date_gen = &mut DateUnixNanoGenerator::from(date!(2023-12-30));
