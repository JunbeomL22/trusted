...
use crate::logger::logger::{local_timezone, LogTimezone};
use crate::logger::logger::info as crate_info;
...
impl FileAppender {
    ...
    ///
    /// ```rust
    /// use trading_engine::logger::appender::{Duration, FileAppender, Period};
    /// use trading_engine::logger::logger::LogTimezone;
    /// use time::UtcOffset;
    ///
    ...
    fn until(period: Period, timezone: &LogTimezone) -> (Minstant, Duration) {
            ...
            if start.elapsed() > *wait {
                ...
                if let Some(keep_duration) = keep {
                    ...
                    std::thread::spawn(move || {
                        ...
                        if !del_msg.is_empty() {
                            crate_info!("Log file deleted: {}", del_msg);
                        }
                    });
                };
