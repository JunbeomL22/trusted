...
impl UdpClient {
    pub fn new(client_address: SocketAddr) -> std::io::Result<Self> {
        ...
        {
            println!("A new UDP client is initialized with its address: {}", client_address);
        }
        Ok(Self { socket, client_address })
    }
...
mod tests {
    ...
    fn test_udp_client() {
        ...
        let server_socket = UdpSocket::bind(server_address).expect("Failed to bind UDP server socket");

        // Spawn a separate thread to handle incoming messages and send a response
        std::thread::spawn(move || {
            let mut buffer = [0; 1024];
            let (bytes_received, origin) = server_socket.recv_from(&mut buffer).expect("Failed to receive data");
            let received_message = std::str::from_utf8(&buffer[..bytes_received]).expect("Failed to read message");
            assert_eq!(received_message, "Hello, server!");

            let response_message = "Hello, client!";
            server_socket.send_to(response_message.as_bytes(), origin).expect("Failed to send response");
        });
        ...
        client.send_message(server_address, message).expect("Failed to send message");

        let response = client.receive_response().expect("Failed to receive response");
