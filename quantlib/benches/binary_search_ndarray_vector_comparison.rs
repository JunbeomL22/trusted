...
fn benchmark(c: &mut Criterion) {
    ...
    let search_vec: Array1<Real> = Array1::from((1600..1800).map(|x| x as Real).step_by(2).collect::<Vec<Real>>());

    group.bench_function("vectorized_search_index_for_sorted_ndarray", |b| {
        b.iter(|| vectorized_search_index_for_sorted_ndarray(black_box(&vec), black_box(&search_vec)))
    });
