...
impl ZeroCurve {
    ...
    pub fn new(evaluation_date: Rc<EvaluationDate>, data: Rc<VectorData>, code: ZeroCurveCode, name: String) -> ZeroCurve {
        ...
        let discount_times = period_leteral.iter().map(|period| add_period(evaluation_date.get_date(), period)).collect();
        let interpolated_rates = rate_interpolator.vectorized_interpolate_for_sorted_input(&discount_times);
        let discount_factors = interpolated_rates.iter().zip(&discount_times).map(|(rate, time)| (-rate * time).exp()).collect();
        
        let discount_interpolator = LinearInterpolator1D::new(&discount_times, &discount_factors, ExtraPolationType::None, false);
    ...
    pub fn get_forward_rate_between_times(&self, t1: Time, t2: Time, compounding: &str) -> Real {
        let disc = self.get_discount_factor_between_times(t1, t2);

        let tau = t2 - t1;
        match compounding {
            "simple" => (disc - 1.0) / tau,
            "continuous" => disc.ln() / tau,
            _ => panic!("The forward rate for the given compounding type has not been defined"),
        }
    }

    pub fn get_forward_rate_between_dates(&self, date1: &OffsetDateTime, date2: &OffsetDateTime, compounding: &str) -> Real {
        ...
        self.get_forward_rate_between_times(t1, t2, compounding).unwrap()
    }
...
impl Parameter for ZeroCurve {
    fn update(&mut self) {
        ...
        self.rate_interpolator = LinearInterpolator1D::new(rate_times.clone(), zero_rates.clone(), ExtraPolationType::Flat, true);

        let interpolated_rates = self.rate_interpolator.vectorized_interpolate_sorted_input(&self.discount_times);
        self.discount_factors = interpolated_rates.iter().zip(&self.discount_times).map(|(rate, time)| (-rate * time).exp()).collect();
        
        self.discount_interpolator = LinearInterpolator1D::new(&self.discount_times, &self.discount_factors, ExtraPolationType::None, false);
    }
}
...
mod tests {
    ...
    fn test_zero_curve() {
        ...
        let data = VectorData::new(vec![0.02, 0.025, 0.03, 0.035, 0.04], Some(dates), vec![], param_dt, "test".to_string());

        let zero_curve = ZeroCurve::new(&evaluation_date, data, ZeroCurveCode::Undefined, "test".to_string());

        assert_eq!(zero_curve.get_discount_factor(0.0), 1.0);
        assert_eq!(zero_curve.get_discount_factor(1.0), 0.9900498337491681);
        assert_eq!(zero_curve.get_discount_factor(2.0), 0.9801986733067553);
        assert_eq!(zero_curve.get_discount_factor(3.0), 0.9704465830709246);
        assert_eq!(zero_curve.get_discount_factor(4.0), 0.9607937460912057);
    }
}
